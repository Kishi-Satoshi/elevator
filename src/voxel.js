...
/* ---------------- フロア編集の保持 ----------------
   ブロック編集はセッション中のみ worldCache に保持し、他階へ行って戻っても残る。
   ただしブラウザをリロードすると初期状態に戻る (localStorage には保存しない)。 */
const SAVE_KEY = 'axiez_base_v2';
function markDirty() { /* 永続化しない: セッション中は worldCache が保持する */ }
function clearPersisted() { try { localStorage.removeItem(SAVE_KEY); } catch (e) { /* noop */ } }
...
/* 地面(grid y=0)の上面を y=0 に合わせる (かご床と面一)。ブロックは下方向に積む。 */
...
function generatePlains() {
  ...
  for (let dx = -pond[2]; dx <= pond[2]; dx++) for (let dz = -pond[2]; dz <= pond[2]; dz++) {
    ...
    setBlock(map, pond[0] + dx, 0, pond[1] + dz, 'dirt'); // 池の底 (支え)
    setBlock(map, pond[0] + dx, 1, pond[1] + dz, 'water'); // 水面 (浸かって歩ける)
  }
  // 石畳の小道 (入口からまっすぐ)
  ...
  for (let dx = -3; dx <= 3; dx++) for (let dz = -3; dz <= 3; dz++) {
    ...
    setBlock(map, -26 + dx, 0, 44 + dz, 'dirt');
    setBlock(map, -26 + dx, 1, 44 + dz, 'water');
  }
  return map;
}
...
export function initVoxel(context) {
  ...
  buildHotbarUI();
  clearPersisted();     // 旧セーブを消去 (リロードでフロアは初期化)
  updateInventoryUI();
}
...
function collideAxis(pos, axis) {
  const feetY = Math.floor((pos.y + .1) / CELL) + 1; // grid y は下面基準 (worldToCell と一致)
  const headY = Math.floor((pos.y + 1.4) / CELL) + 1;
...
function groundHeightAt(x, z) {
  ...
  for (let y = GRID.yMax; y >= 0; y--) {
    if (solidAt(c.x, y, c.z)) return y * CELL; // grid y の上面 (かご床基準)
  }
...
export function updateVoxel(dt, walkMode, t) {
  ...
  const sneak = isSneak();
  // 立脚セル(足元)とすねセルの両方で液体を判定 (溶岩の上に立つ / 水に浸かる)
  const groundCell = worldToCell(new THREE.Vector3(player.pos.x, player.pos.y - .05, player.pos.z));
  const shinCell = worldToCell(new THREE.Vector3(player.pos.x, player.pos.y + .4, player.pos.z));
  const gType = typeAt(groundCell.x, groundCell.y, groundCell.z);
  const sType = typeAt(shinCell.x, shinCell.y, shinCell.z);
  const inWater = gType === 'water' || sType === 'water';
  const inLava = gType === 'lava' || sType === 'lava';
